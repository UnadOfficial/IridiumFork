...
namespace Iridium.Patches
{
    public static class PatchManager
    {
        ...
        private static void RegisterPatches()
        {
            ...
            // 排除分帧加载和 no-op StartEffect probes，它们有独立条件或已由其他补丁覆盖。
            RegisterNestedPatches(typeof(LoadingOptimizationPatches), optCond,
                new HashSet<Type>
                {
                    typeof(LoadingOptimizationPatches.FrameSpreadDecorationLoadingPatch),
                    typeof(LoadingOptimizationPatches.MoveTrackTweenOptimizationPatch),
                    typeof(LoadingOptimizationPatches.MoveDecorationBatchOptimizationPatch)
                });
